...
function Phone({ id, brand, price, quantity, storageGB, isAdmin, onDelete, onUpdate }) {
    const [isEditing, setIsEditing] = useState(false);
    const [tempBrand, setTempBrand] = useState(brand);
    const [tempPrice, setTempPrice] = useState(price);
    const [tempQuantity, setTempQuantity] = useState(quantity);
    const [tempStorageGB, setTempStorageGB] = useState(storageGB);
    ...
    const handleSave = () => {
        const updated = {
            ...
            storageGB: parseInt(tempStorageGB)
        };
        onUpdate(id, updated);
        setIsEditing(false);
    };
            ...
            <div style={styles.editForm}>
                <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
                    ...
                    <input
                        type="number"
                        value={tempStorageGB}
                        onChange={(e) => setTempStorageGB(e.target.value)}
                        style={{ ...styles.editInput, width: '100px' }}
            ...
            <div style={styles.content}>
                <div style={styles.info}>
                    ...
                    <p style={styles.details}>
                        <strong>Storage:</strong> {storageGB}GB | <strong>Price:</strong> <span style={styles.price}>${price.toFixed(2)}</span> | <strong>Quantity:</strong> {quantity}
                    </p>
                </div>
                {isAdmin && (
